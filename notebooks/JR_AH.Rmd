...
#square_footage, age of home at time of sale, group, year, tract. How can you interpret the coefficients of this model?
```{r}
reg1 <- lm(amount ~ square_footage + age + group + sale_yr + tract,
           data = nearli_5yr_sales)
summary(reg1)
```
# Coefficient interpretation: sale amount = -1.346e+07 + 55.48(square footage) - 605.3(house age) - (value associated w/ relevant group) + 7217(sale_yr) - (value associated w/ relevant tract)

# 10. Now, try a model with target being the log of the sale price: square_footage, age of home at time of sale, group, year, tract.
```{r}
reg2 <- lm(log(amount) ~ square_footage + age + group + sale_yr + tract,
              data = nearli_5yr_sales)
summary(reg2)
```
#How can you interpret the coefficients of this model? 
# Coefficient interpretation: log(sale amount) = -76.73 + 0.0002679(square footage) - 0.001777(house age) + (value associated w/ relevant group*) + 0.04410(sale_yr) + (value associated w/ relevant tract*)
# * some values are positive, others are negative
